...
const AuthPage = (props) => {
 ...
 const navigate =useNavigate();
  const onLogin = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:8080/api/auth/chat/login", { username, secret })
      .then((r) => {
        props.onAuth({ ...r.data, secret }); // NOTE: over-ride secret
       navigate(`/chatroom`); // Navigate to /dashboard on successful login
      })
  ...
  const onSignup = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:8080/api/auth/chat/signup", {
        username,
        secret,
        email,
        first_name,
      ...
      .then((r) => {
        props.onAuth({ ...r.data, secret }); // NOTE: over-ride secret
        navigate(`/chatroom`); // Navigate to /dashboard on successful signup
      })
      .catch((e) => console.log(JSON.stringify(e.response.data)));
  };
